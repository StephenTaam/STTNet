...
namespace stt
{
    ...
    */
    ...
    public:
        ...
        * @endcode
        */
        bool openFile(const std::string &fileName,const bool &create=true,const int &multiple=0,const size_t &size=0,const mode_t &mode=0666);如果multipl
        /** 关闭文件
        * @brief 关闭已打开了的文件
    ...
    */
    class LogFile:private DateTime,protected File
    {
    private:
    ...
    */
    ...
    public:
        ...
        * @param dt 填入监听超时时间
        */
        void startListen(const int &fd,const bool &flag=true,const Duration &dt=Duration{0,0,20,0,0});
    public:
        /**
        * @brief 返回epoll监听状态
        ...
        */
        void waitAndQuit(const Duration &t=Duration{0,0,0,10,10}){flag3=true;this->t=t;}
        /**
    ...
    */
    namespace system
    {
        ...
        */
        ...
        private:
            static void signalterminated(){std::cout<<"未捕获的异常终止"<<std::endl;if(ServerSetting::logfile!=nullptr){if(ServerSetting::language=="Chinese")ServerSetting::logfile->writeLog("未捕获的异常终止");else ServerSetting::logfile->writeLog("end for uncaught exception");}kill(getpid(),15);}
        ...
        public:
            ...
            */
            static void setLogFile(LogFile *logfile=nullptr,const std::string &language="");
            /**
            * @brief 执行setExceptionHandling和setLogFile两个函数，完成初始化信号和日志系统
            * @param logfile 传入日志文件对象的指针，如果这个对象没初始化，系统自动在程序目录下生成server_log文件夹并且根据当前时间生成一个日志文件。 （默认为nullptr，不设置日志文件）
            * @param language 日志文件的语言 "Chinese":中文 除此之外都会设置成英文 （默认为空 设置为英文）
            */
            static void init(LogFile *logfile=nullptr,const std::string &language="");
        };
